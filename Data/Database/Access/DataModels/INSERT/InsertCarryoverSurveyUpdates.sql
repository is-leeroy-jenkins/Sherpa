...
SELECT Transfers.BFY                                             AS BFY,
	   ...
	   Nz( Switch(
					   [BocCode] = 38, 06 G,
					   [BocCode] <> 10 AND
					   [FundCode] ALike E% OR [BocCode] <> 10 AND [FundCode] = B And [NpmCode]=B,
					   06 K,
					   [BocCode] <> 10 AND [FundCode] = B OR [BocCode] <> 10 AND
															 [FundCode] ALike F% OR [BocCode] <> 10 AND [FundCode] ALike E% And [NpmCode] In (C,D,E),
					   06 B,
					   [BocCode] <> 10 AND
					   [AhCode] ALike 6A% OR [BocCode] <> 10 AND [FundCode] ALike T% AND [NpmCode]<>F AND [NpmCode] <> Y,
					   06 L,
					   [BocCode] <> 10 AND
					   [FundCode] ALike E% OR [BocCode] <> 10 AND [FundCode]=B And [NpmCode]=A,
					   06 J,
					   [BocCode] <> 10 AND [FundCode] = B And [NpmCode] = E, 06 M,
					   [BocCode] <> 10 AND [NpmCode] = L, 06 F,
					   [BocCode] <> 10 AND
					   [AccountCode] ALike %57 Or [BocCode] <> 10 AND [AccountCode] ALike %M90,
					   06 F,
					   [BocCode] <> 10 AND [NpmCode] = F OR [BocCode] <> 10 AND
															[AccountCode] ALike %XW2 OR [BocCode] <> 10 AND [AccountCode] ALike %RL,
					   06 H,
					   [BocCode] <> 10 AND [NpmCode] = Y OR [BocCode] <> 10 AND
															[AccountCode] ALike %02 OR [BocCode] <> 10 AND [AccountCode] ALike %04 OR [BocCode] <> 10 AND [AccountCode] ALike %03,
					   06 N ), 06 )                              AS RcCode,
	   ...
	   IIf( [DocPrefix] <> CRP, Left( [ControlNumber], 2 ), 17 ) AS SPIO,
...
FROM Transfers
GROUP BY Transfers.ReprogrammingNumber, Transfers.ProcessedDate,
		 IIf( [DocPrefix] <> CRP, Left( [ControlNumber], 2 ), 17 ), Transfers.BFY,
		 Transfers.RpioCode, Transfers.AhCode, Transfers.AhName, Transfers.FundCode,
		 Transfers.FundName, Transfers.OrgCode, Transfers.OrganizationName, Transfers.AccountCode,
...
HAVING ((Transfers.DocPrefix) = CRP)
