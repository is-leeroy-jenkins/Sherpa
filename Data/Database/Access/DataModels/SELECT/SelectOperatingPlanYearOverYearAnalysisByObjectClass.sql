SELECT DISTINCTROW Allocations.PrcId, Allocations.BFY,
	   ...
	   CCur( (Round( SystemUpdates.Amount, 2 )) )        AS System,
	   CCur( (0 - Round( Nz( [Allocations].[Amount], 2 ) - Round( SystemUpdates.Amount, 2 ),
						 2 )) )                          AS Delta,
	   IIf( [Delta] < -1, "DECREASE",
			IIf( [Delta] > 1, "INCREASE", "NET ZERO" ) ) AS NET
FROM Allocations
		 LEFT JOIN SystemUpdates ON (Allocations.RcCode = SystemUpdates.RcCode) AND
									(Allocations.AccountCode = SystemUpdates.AccountCode) AND
									(Allocations.OrgCode = SystemUpdates.OrgCode) AND
									(Allocations.BudgetLevel = SystemUpdates.BudgetLevel) AND
									(Allocations.FundCode = SystemUpdates.FundCode) AND
									(Allocations.AhCode = SystemUpdates.AhCode) AND
									(Allocations.BFY = SystemUpdates.BFY)
...
GROUP BY Allocations.PrcId, Allocations.BFY, Allocations.BudgetLevel, Allocations.AhCode,
		 ...
		 CCur( (Round( SystemUpdates.Amount, 2 )) ),
		 CCur( (0 - Round(
					 Nz( [Allocations].[Amount], 2 ) - Round( SystemUpdates.Amount, 2 ), 2 )) ),
		 IIf( (0 - Round( Nz( [Allocations].[Amount], 2 ) - Round( SystemUpdates.Amount, 2 ), 2 )) <
			  0, "DECREASE",
			  IIf( (0 - Round( Nz( [Allocations].[Amount], 2 ) - Round( SystemUpdates.Amount, 2 ),
							   2 )) > 0, "INCREASE",
				   "NET ZERO" ) )
