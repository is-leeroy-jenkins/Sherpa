...
SELECT DISTINCT Employees.RcCode,
                DLookUp( "Title", "Divisions", "Divisions.Code = '" & RcCode &"'" ) AS DivisionName,
                ...
                DLookUp( "LastName", "EmployeeData",
                         "EmployeeData.EmployeeId = '" & [SupervisorId]&"'" )       AS SupervisorLastName,
                DLookUp( "FirstName", "EmployeeData",
                         "EmployeeData.EmployeeId = '" & [SupervisorId]&"'" )       AS SupervisorFirstName, LEFT (SupervisorFirstName, 1) & ". " & SupervisorLastName AS Supervisor
FROM LocatorData INNER JOIN (Employees INNER JOIN EmployeeData ON Employees.EpaNumber = EmployeeData.EmployeeId)
ON (LocatorData.FirstName = EmployeeData.FirstName) AND (LocatorData.LastName = EmployeeData.LastName)
