INSERT INTO ProgrammaticReserve (PrcId, BudgetLevel, RPIO, AhCode, BFY, FundCode, OrgCode,
                                 AccountCode, ActivityCode, BocCode, RcCode, ProgramProjectCode,
                                 ProgramProjectName, CurrentYear, PriorYear, Reduction, Ratio,
                                 Budget, FundName, BocName, NpmCode, NpmName, Division,
                                 DivisionName)
SELECT Allocations.PrcId,
       ...
       Round( Sum( [Allocations].[Amount]), 0 )                                           AS CurrentYear,
       Nz( DLookUp( "Amount", "Allocations",
                    "Allocations.BFY = '2020'" & " AND Allocations.AhCode= '" & [Allocations].[AhCode] & "'" & " AND Allocations.OrgCode = '" & [Allocations].[OrgCode] & "'" & " AND Allocations.FundCode = '" & [Allocations].[FundCode] & "'" & " AND Allocations.AccountCode = '" & [Allocations].[AccountCode] & "'" & " AND Allocations.BocCode = '" & [Allocations].[BocCode] & "'" & " AND Allocations.RcCode = '" & [Allocations].[RcCode] & "'" ),
           0 )                                                                            AS PriorYear,
       Round( Sum( Allocations.Amount * 0.00772028905587925 ), 3 )                        AS Reduction,
       IIf( Allocations.Amount > 0,
            Round( Sum( Allocations.Amount * 0.00772028905587925 ) / Sum( Allocations.Amount ), 3 ),
            0 )                                                                           AS Ratio,
       Round( Sum( Allocations.Amount - (Allocations.Amount * 0.00772028905587925) ), 0 ) AS Budget,
...
WHERE (((Allocations.BudgetLevel) = '8')
    ...
    AND ((Allocations.FundCode)
    Not ALike "E%"
  AND (Allocations.FundCode) Not ALike "%R%")
  AND Allocations.ProgramProjectCode NOT IN ('89'
    , '68')
