		...
		INSERT INTO Allocations (RPIO, BudgetLevel, AhCode, BFY, FundCode, OrgCode, AccountCode,
								 ...
								 NpmName, ProgramProjectName, ProgramAreaCode, ProgramAreaName,
								 GoalName,
		...
		SELECT Allocations.RPIO,
			   ...
			   Nz( DLookUp( "Name", "NationalPrograms",
							"NationalPrograms.Code=MID([AccountCode], 3, 1)" ),
				   "NS" )                                           AS NpmName,
			   Nz( DLookUp( "Name", "ProgramProjects",
			   ...
			   Nz( DLookUp( "ObjectiveName", "Allocations",
							"Allocations.ObjectiveCode=[ObjectiveCode]" ),
				   "NS" )                                           AS ObjectiveName,
		...
		FROM (((Allocations INNER JOIN (Divisions INNER JOIN (ProgramProjects INNER JOIN Accounts
										...
										ON Divisions.Code = ProgramProjects.Code) ON
