PARAMETERS
pBudgetLevel Text ( 255 ), pRPIO Text ( 255 ), pAhCode Text ( 255 ), pBFY Text ( 255 ), pFundCode Text ( 255 ), pOrgCode Text ( 255 ), pAccountCode Text ( 255 ), pBocCode Text ( 255 ), pRcCode Text ( 255 ), pAmount IEEEDouble;
...
SELECT Allocations.RPIO,
       ...
       Nz( DLookUp( "ObjectiveName", "Allocations", "Allocations.ObjectiveCode=[ObjectiveCode]" ),
           "NS" )                                                        AS ObjectiveName,
       Nz( Switch( [BudgetLevel]='7',1, [BudgetLevel]='8',0 ), 0 )       AS AllocationRatio
FROM (((Allocations INNER JOIN (Divisions INNER JOIN (ProgramProjects INNER JOIN Accounts ON ProgramProjects.Code = Accounts.ProgramProjectCode) ON Divisions.Code = ProgramProjects.Code) ON
        (Allocations.ProgramAreaCode = ProgramProjects.ProgramAreaCode) AND
...
WHERE (((Allocations.RPIO) = "06")
    AND ((Allocations.BudgetLevel) =
    [pBudgetLevel])
  AND ((Allocations.AhCode)=[pAhCode])
  AND ((Allocations.BFY)=[pBFY])
  AND ((DLookUp("Code"
    , "Funds"
    , "Funds.Code=[pFundCode]"))=[pFundCode])
  AND ((Allocations.OrgCode)=[pOrgCode])
  AND ((DLookUp("Code"
    , "Accounts"
    , "Accounts.Code=[pAccountCode]"))=[pAccountCode])
