PARAMETERS
BudgetLevelArgs Text ( 255 ), RpioArgs Text ( 255 ), AhCodeArgs Text ( 255 ), FiscalYearArgs Text ( 255 ), FundCodeArgs Text ( 255 ), OrgCodeArgs Text ( 255 ), AccountCodeArgs Text ( 255 ), 
...
SELECT Allocations.RPIO,
       ...
       Nz( IIf( Len( [AccountCodeArgs]) > 7, Right([AccountCodeArgs],2),"NS" ) ) AS ActivityCode,
       Allocations.BocCode,
       Allocations.RcCode,
       Allocations.Amount,
...
WHERE (((Allocations.RpioArgs) = '06')
  ...
  AND ((DLookUp("Code"
    , "Funds"
    , "Funds.Code=[FundCodeArgs]")) = [FundCodeArgs])
  AND ((Allocations.OrgCode) = [OrgCodeArgs])
  AND ((DLookUp("Code"
    , "Accounts"
    , "Accounts.Code=[AccountCodeArgs]")) = [AccountCodeArgs])
